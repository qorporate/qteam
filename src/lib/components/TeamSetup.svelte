...
<div class="flex flex-col gap-4">
	...
	<section class="flex flex-col gap-4" aria-labelledby="team-count-heading">
		...
		<fieldset class="grid grid-cols-2 gap-3 sm:gap-4">
			<legend class="sr-only">Choose the number of teams</legend>
			{#each validTeamCounts as count (count)}
				<button
					class={[
						'flex min-h-28 flex-col items-start justify-between gap-3 rounded-2xl border p-4 text-left transition-[background-color,border-color,transform] duration-150 ease-out focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--color-ink) active:scale-[0.96] motion-reduce:active:scale-100',
						teamCount === count
